...
// Fix for Safari
	...
	jQuery('.header_mobile .menu_main_nav, .menu_side_responsive, .menu_pushy_nav_area, body:not(.woocommerce) .widget_area:not(.footer_wrap) .widget_product_categories').on('click', 'li a,li a .open_child_menu, ul.product-categories.plain li a .open_child_menu', function(e) {
		...
		var KESa = jQuery(this).hasClass('open_child_menu') ? jQuery(this).parent() : jQuery(this);
		if ((!is_menu_main || body.hasClass('menu_mobile')) && (KESa.parent().hasClass('menu-item-has-children') || KESa.parent().hasClass('has_children'))) {
			if (KESa.siblings('ul:visible').length > 0)
				KESa.siblings('ul').slideUp().parent().removeClass('opened');
			else {
				jQuery(this).parents('li').siblings('li').find('ul:visible').slideUp().parent().removeClass('opened');
				KESa.siblings('ul').slideDown().parent().addClass('opened');
			}
		}
		// Ignore link for parent menu items
		if (jQuery(this).hasClass('open_child_menu') || KESa.attr('href')=='#') {
			e.preventDefault();
			return false;
		}
	});
...
function hotcoffee_stretch_width() {
	jQuery('.trx-stretch-width').each(function() {
		var KESel = jQuery(this);
		var KESel_full = KESel.next('.trx-stretch-width-original');
		var el_margin_left = parseInt( KESel.css( 'margin-left' ), 10 );
		var el_margin_right = parseInt( KESel.css( 'margin-right' ), 10 );
		var offset = 0 - KESel_full.offset().left - el_margin_left;
		var width = jQuery( window ).width();
		if (!KESel.hasClass('inited')) {
			KESel.addClass('inited invisible');
			KESel.css({
				'position': 'relative',
		...
		if ( !KESel.hasClass('trx-stretch-content') ) {
			var padding = Math.max(0, -1*offset);
			var paddingRight = Math.max(0, width - padding - KESel_full.width() + el_margin_left + el_margin_right);
			KESel.css( { 'padding-left': padding + 'px', 'padding-right': paddingRight + 'px' } );
		}
		KESel.removeClass('invisible');
	});
}
...
function hotcoffee_init_post_formats() {
	...
	if (HOTCOFFEE_STORAGE['popup_engine'] === 'pretty') {
		jQuery("a[hrefKES='jpg'],a[hrefKES='jpeg'],a[hrefKES='png'],a[hrefKES='gif']").attr('rel', 'prettyPhoto[slideshow]');
		...
		jQuery("a[hrefKES='jpg'],a[hrefKES='jpeg'],a[hrefKES='png'],a[hrefKES='gif']").attr('rel', 'magnific');
